...
BUILD_ROOT = Path(
    os.environ.get(
        "RTL_BUILD_ROOT",
        str(Path(tempfile.gettempdir()) / "cornell_state_estimation_rtl") if os.name == "nt" else str(ROOT / ".rtl_build"),
    )
)
...
def verilator_command() -> tuple[list[str], dict[str, str]]:
    ...
    if os.name == "nt":
        ...
        if binary.exists():
            ...
            environment["VERILATOR_ROOT"] = str(msys_root / "ucrt64" / "share" / "verilator")
            return [str(binary)], environment
    raise RuntimeError(
        "Verilator was not found. On Windows run tools/install_rtl_tools.ps1, "
        "or set VERILATOR to the executable path."
    )
...
def run_command(command: list[str], environment: dict[str, str], cwd: Path = ROOT) -> None:
    print("+", subprocess.list2cmdline(command), flush=True)
    subprocess.run(command, cwd=cwd, env=environment, check=True)


def tool_path(path: Path) -> str:
    """Use forward slashes so generated C++ source locations are escaped correctly."""
...
def main() -> int:
    ...
    parser.add_argument("--test", help="run testbenches whose module name contains this text")
    parser.add_argument("--list", action="store_true", help="list discovered testbenches and exit")
    parser.add_argument("--lint-only", action="store_true", help="lint synthesizable RTL without running testbenches")
    parser.add_argument("--trace", action="store_true", help="enable VCD tracing in generated models")
    ...
    if arguments.lint_only:
        run_command(verilator + common_options + ["--lint-only", *map(tool_path, rtl_sources)], environment)
